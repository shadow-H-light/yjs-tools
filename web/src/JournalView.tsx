...
function formatImpactFactor(journal: Journal): string {
  const value = journal.official?.impact_factor;
  if (value == null) return "暂无";
  const year = journal.official?.year;
  return year != null ? `${value}（${year} 导入）` : String(value);
}

function formatQuartile(journal: Journal): string {
  const bits: string[] = [];
  if (journal.official?.jcr_quartile != null) {
    bits.push(`JCR Q${journal.official.jcr_quartile}`);
  }
  if (journal.official?.cas_quartile != null) {
    bits.push(`中科院 ${journal.official.cas_quartile} 区`);
  }
  return bits.length ? bits.join(" · ") : "暂无";
}

function formatJournalNumber(journal: Journal): string {
  const issn = journal.issn_l?.trim();
  const oid = journal.openalex_id?.trim();
  if (issn && oid) return `${issn} · ${oid}`;
  if (issn) return issn;
  if (oid) return `暂无 ISSN · ${oid}`;
  return "暂无";
}
...
export default function JournalView() {
  ...
  async function onIngest() {
    ...
    setStatus(null);
    setActivity(
      "正在全面同步 OpenAlex 期刊库，并重试补全 ISSN、主页、被引。可能需要几分钟，请勿关闭页面。",
    );
    try {
      const result = await api<{
        ...
        completed?: number;
        missing_issn?: number;
        missing_homepage?: number;
        stats: Stats;
      }>(`/api/ingest?limit=800&scope=${ingestScope}`, { method: "POST" });
      const missing: string[] = [];
      if (result.missing_issn) missing.push(`ISSN ${result.missing_issn} 种`);
      if (result.missing_homepage) missing.push(`主页 ${result.missing_homepage} 种`);
      setStatus(
        `同步完成：新增 ${result.inserted}，更新 ${result.updated}，合计 ${result.total}` +
          `。库中现有 ${result.stats.journals} 种。` +
          (missing.length
            ? ` OpenAlex 仍缺 ${missing.join("、")}，列表会显示暂无。影响因子/分区需导入 CSV。`
            : " 影响因子/分区仍以导入的官方表为准。"),
      );
      await refreshMeta();
      await runSearch(query);
  ...
  return (
    <>
      ...
      <section className="toolbar">
        <form className="toolbar-group" onSubmit={(e) => { e.preventDefault(); onIngest(); }}>
          ...
          <button type="submit" className="secondary" disabled={busy}>
            {busy && activity?.includes("同步") ? "同步中…" : "全面同步期刊库"}
          </button>
        </form>
        <form className="toolbar-group import-form" onSubmit={onImport}>
          <div className="field-help">
            ...
            <p>
              选择 CSV 或科睿唯安 JCR 的 xlsx（需有 ISSN）。按刊号匹配已入库期刊，写入影响因子和分区。
            </p>
            <input
              id="metrics-file"
              type="file"
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                <th>期刊编号</th>
                {compared.map((journal) => (
                  <td key={journal.id}>{formatJournalNumber(journal)}</td>
                ))}
              </tr>
              <tr>
                <th>影响因子</th>
                {compared.map((journal) => (
                  <td key={journal.id}>{formatImpactFactor(journal)}</td>
                ))}
              </tr>
              <tr>
                <th>被引次数</th>
              ...
              <tr>
                <th>分区</th>
                {compared.map((journal) => (
                  <td key={journal.id}>{formatQuartile(journal)}</td>
                ))}
              </tr>
              {compared.some((journal) => journal.official?.review_days != null) && (
                <tr>
                  <th>审稿时长</th>
                  {compared.map((journal) => (
                    <td key={journal.id}>
      ...
      {allResults.length > 0 && sort === "impact_factor" && (
        <p className="hint">
          影响因子排序只用你导入的 JCR IF。未导入的刊排在后面，卡片上会写「影响因子 暂无」。
        </p>
      )}
      ...
      {results.map((journal) => (
        <article key={journal.id}>
          ...
          <div className="facts">
            ...
            {journal.is_oa && <span className="badge">OA</span>}
            <span>{journal.publisher || "出版社未知"}</span>
            {journal.official?.warning && <span className="badge">预警</span>}
          </div>
          <dl className="required-facts">
            <div>
              <dt>影响因子</dt>
              <dd>{formatImpactFactor(journal)}</dd>
            </div>
            <div>
              <dt>被引次数</dt>
              <dd>{journal.cited_by_count.toLocaleString()}</dd>
            </div>
            <div>
              <dt>分区</dt>
              <dd>{formatQuartile(journal)}</dd>
            </div>
            <div>
              <dt>期刊编号</dt>
              <dd>{formatJournalNumber(journal)}</dd>
            </div>
            <div>
              <dt>期刊链接</dt>
              <dd>
            ...
            {journal.official?.review_days != null && (
              <div>
                <dt>审稿时长</dt>
                <dd>{journal.official.review_days} 天</dd>
              </div>
            )}
            {journal.citedness_2yr != null && (
              <div>
                <dt>两年篇均被引</dt>
                <dd>{journal.citedness_2yr.toFixed(1)}（OpenAlex）</dd>
              </div>
            )}
          </dl>
